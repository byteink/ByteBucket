...
func TestE2E_Stats(t *testing.T) {
	...
	if s.Buckets < 1 || s.Objects < 1 || s.Bytes < 5 {
		t.Fatalf("stats too low: %+v", s)
	}
	// The object PUT above must register as upload activity, both in the total
	...
	for _, b := range s.PerBucket {
		if b.Name == bucket {
			found = true
			if b.Uploads < 1 {
				t.Fatalf("seeded bucket %q has no recorded upload: %+v", bucket, b)
			}
		}
	}
	if !found {
		t.Fatalf("seeded bucket %q not in perBucket %+v", bucket, s.PerBucket)
	}
}
