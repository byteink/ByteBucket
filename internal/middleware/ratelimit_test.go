...
func TestRateLimitMiddlewareThrottles(t *testing.T) {
	cfg := RateLimitConfig{Enabled: true, RPS: 1, Burst: 2}
...
// evasion guard: with no trusted headers, rotating a spoofed X-Forwarded-For
...
func TestControllerCurrent(t *testing.T) {
	rc := NewRateLimitController(RateLimitConfig{Enabled: false, RPS: 2, Burst: 3})
	t.Cleanup(rc.rl.close)
	if got := rc.Current(); got.RPS != 2 || got.Burst != 3 || got.Enabled {
		t.Fatalf("Current() = %+v, want seeded env config", got)
	}
	want := RateLimitConfig{Enabled: true, RPS: 9, Burst: 4}
	rc.Apply(want)
...
// behind-a-proxy deployment (X-Forwarded-For trusted, rightmost). The trusted
// proxy always appends the genuine client to the right; the attacker controls
// only the prepended prefix and rotates it each request. Both requests must key
// on the same client, so the second trips the limiter. This pins the rightmost
// choice: a regression that selected the leftmost (spoofed) entry would mint a
// fresh bucket per request and this test would see a 200 instead of a 503.
